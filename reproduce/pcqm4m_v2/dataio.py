...
def shortest_path_length2(edge_index, num_nodes):
    ...
    return dist.astype('int16'), paths
...
def atom_same_ring_count(mol):
    ...
    return np.sum(atom_groups[:, None, :] * atom_groups[None, :, :], axis=-1)
...
def smiles2graph(s):
    ...
    dist, paths = shortest_path_length2(
...
def transform_graph_preprocess(data):
    ...
    graph['atom_feat_float'] = graph['atom_feat_float'][:, :6]
    return graph, y
