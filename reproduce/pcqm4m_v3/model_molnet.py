...
class GNNLayer(nn.Module):
    def __init__(self, config):
        ...
        self.layer_embed_same_ring_size_min = nn.Sequential(
            nn.Embedding(
                config['max_same_ring_min_size'], head_size, padding_idx=0),
            nn.LayerNorm(head_size))
        ...
        self.layer_paths2_map = nn.Linear(head_size, head_size)
        self.layer_paths2_atom_map = nn.Linear(
            config['hidden_size'], head_size)
        self.layer_paths3_atom_map = nn.Linear(
            config['hidden_size'], head_size)
        pass
    ...
    def forward(
            ...
            same_ring_count, same_ring_size_min,
            paths2, paths2_atom, paths3_atom
    ):
        ...
        paths2_emb = self.batch_index_select(
            self.layer_paths2_map(hidden_bond), paths2)
        paths2_emb[paths2_mask] = 0

        paths2_atom = torch.unsqueeze(paths2_atom, -1)
        paths2_atom_emb = self.batch_index_select(
            self.layer_paths2_atom_map(hidden_node), paths2_atom)

        paths3_mask = torch.any(paths3_atom == -1, dim=-1)
        paths3_atom_emb = self.batch_index_select(
            self.layer_paths3_atom_map(hidden_node), paths3_atom)
        paths3_atom_emb[paths3_mask] = 0

        edge_emb[:, 1:, 1:] += self.layer_embed_path_length(
            shortest_path_length) + self.layer_embed_same_ring_count(
                same_ring_count) + (
                    paths2_emb + paths2_atom_emb + paths3_atom_emb)
...
class MoleculeNet(nn.Module):
    ...
    def forward(
            ...
            edge_aa_bond,
            shortest_path_length, same_ring_count, same_ring_size_min,
            paths2, paths2_atom, paths3_atom,
        ...
        for i in range(len(self.spread_layers)):
            hidden_node = self.spread_layers[i](
                ...
                same_ring_count, same_ring_size_min, paths2, paths2_atom,
                paths3_atom
            )
            pass
